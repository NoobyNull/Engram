...
export function getObservationsBySession(sessionId: string, limit?: number): Observation[] {
  const db = getDb();
...
export function getObservationsByConversation(conversationId: string, limit?: number): Observation[] {
  const db = getDb();
...
export function getRecentObservations(projectPath: string, limit: number = 20): Observation[] {
  const db = getDb();
...
export function getObservationsAround(timestamp: number, limit: number = 20, projectPath?: string): Observation[] {
  const db = getDb();
  if (projectPath) {
    const rows = db.prepare(`
      SELECT * FROM observations WHERE project_path = ?
      ORDER BY ABS(timestamp - ?) ASC LIMIT ?
    `).all(projectPath, timestamp, limit) as Record<string, unknown>[];
    return rows.map(deserializeObservation);
  }
  const rows = db.prepare(`
    SELECT * FROM observations ORDER BY ABS(timestamp - ?) ASC LIMIT ?
  `).all(timestamp, limit) as Record<string, unknown>[];
  return rows.map(deserializeObservation);
}
...
export function deleteObservationsByQuery(query: string, beforeDate?: number): number {
  const db = getDb();
...
export function countObservations(projectPath?: string): number {
  const db = getDb();
  if (projectPath) {
    const row = db.prepare('SELECT COUNT(*) as count FROM observations WHERE project_path = ?').get(projectPath) as { count: number };
    return row.count;
  }
...
function deserializeObservation(row: Record<string, unknown>): Observation {
  return {
    id: row['id'] as string,
    session_id: row['session_id'] as string,
    conversation_id: row['conversation_id'] as string | null,
    tool_name: row['tool_name'] as string,
    tool_input_summary: row['tool_input_summary'] as string | null,
    tool_output_summary: row['tool_output_summary'] as string | null,
    project_path: row['project_path'] as string,
    files_involved: JSON.parse((row['files_involved'] as string) || '[]'),
