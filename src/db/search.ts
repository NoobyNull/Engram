...
function searchFts(query: string, options: SearchOptions): FtsResult[] {
  const db = getDb();
    ...
    try {
      ...
      for (const row of rows) {
        results.push({
          id: row['id'] as string,
          type: 'observation',
          snippet: truncate((row['tool_input_summary'] as string) || (row['tool_output_summary'] as string) || '', 200),
    ...
    try {
      ...
      for (const row of rows) {
        results.push({
          id: row['id'] as string,
          type: 'session',
          snippet: truncate((row['summary'] as string) || '', 200),
          rank: row['rank'] as number,
          timestamp: row['started_at'] as number,
          project_path: row['project_id'] as string | null,
          tags: '[]',
        });
      }
    } catch (err) {
    ...
    try {
      ...
      for (const row of rows) {
        results.push({
          id: row['id'] as string,
          type: 'conversation',
          snippet: truncate((row['topic'] as string) || (row['summary'] as string) || '', 200),
          rank: row['rank'] as number,
          timestamp: row['started_at'] as number,
          project_path: row['project_path'] as string | null,
          tags: '[]',
...
export async function hybridSearch(
  ...
  const resultMap = new Map<string, FtsResult>();
  for (const r of ftsResults) {
  ...
  for (const id of allIds) {
    ...
    if (options.tags && options.tags.length > 0 && ftsResult) {
      const itemTags: string[] = JSON.parse(ftsResult.tags || '[]');
