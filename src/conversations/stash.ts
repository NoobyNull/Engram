 ...
 */
export function getStashedSidebars(
  projectPath?: string,
): Array<{ conversation: Conversation; group: StashGroup | null }> {
  ...
  const groupMap = new Map<string, StashGroup>();
 ...
 */
export async function assignToStashGroup(
  conversationId: string,
  projectPath: string,
): Promise<string | null> {
  // We import getConversation lazily to avoid a circular dep at module level.
  const { getConversation, stashConversation: stashConv } = await import(
    '../db/conversations.js'
  );
