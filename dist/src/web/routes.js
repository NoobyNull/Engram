...
let globalStagingBuffer = null;
export function setStagingBuffer(buffer) {
...
export async function handleApiRequest(req, res) {
    ...
    if (method === 'GET' && pathname === '/api/sessions/active') {
        const db = getDb();
        const rows = db.prepare(`SELECT s.id, s.claude_session_id, s.project_id, s.started_at, s.observation_count,
              p.name AS project_name, p.root_path AS project_path
       FROM sessions s
       LEFT JOIN projects p ON s.project_id = p.id
       WHERE s.ended_at IS NULL
       ORDER BY s.started_at DESC`).all();
    ...
    if (method === 'GET' && pathname === '/api/sessions') {
        ...
        if (project) {
            ...
            const db = getDb();
            const projRow = db.prepare('SELECT id FROM projects WHERE root_path = ?').get(project);
            if (projRow) {
    ...
    if (method === 'GET' && pathname === '/api/knowledge') {
        ...
        if (sessionFilter) {
            ...
            const db = getDb();
            const convIds = db.prepare('SELECT id FROM conversations WHERE session_id = ?').all(sessionFilter);
