...
function scoreFileOverlap(contextFiles, activityFiles) {
    if (contextFiles.size === 0 && activityFiles.size === 0)
        return 0; // no signal
    if (contextFiles.size === 0 || activityFiles.size === 0)
        return 0.5; // partial signal
    let overlap = 0;
    for (const f of activityFiles) {
        if (contextFiles.has(f))
            overlap++;
    }
...
function scoreDirectoryProximity(contextFiles, activityFiles) {
    ...
    if (contextDirs.size === 0 && activityDirs.size === 0)
        return 0;
    if (contextDirs.size === 0 || activityDirs.size === 0)
        return 0.5;
    let overlap = 0;
    for (const d of activityDirs) {
        if (contextDirs.has(d))
            overlap++;
    }
...
function scoreTimeGap(lastObservationTimestamp) {
    if (!lastObservationTimestamp)
        return 0; // no data — assume same
    const gapMs = Date.now() - lastObservationTimestamp;
    const gapMinutes = gapMs / 60_000;
    if (gapMinutes < 0.5)
        return 0; // < 30 seconds
    if (gapMinutes < 2)
        return 0.1; // < 2 minutes
    if (gapMinutes < 5)
        return 0.25; // < 5 minutes
    if (gapMinutes < 10)
        return 0.5; // < 10 minutes
    if (gapMinutes < 30)
        return 0.75; // < 30 minutes
...
function scoreToolPatternChange(recentTools, newActivity) {
    ...
    if (recentIsCode && !recentIsResearch && mentionsUrl && !mentionsFile)
        return 0.7;
    if (recentIsResearch && !recentIsCode && mentionsFile && !mentionsUrl)
        return 0.6;
...
function scorePromptStructure(newActivity) {
    ...
    if (continuationRe.test(trimmed))
        return 0.1;
    return 0.5;
}
/**
 ...
 * adaptive thresholds.
 */
export function scoreTopicShift(ctx) {
    ...
    // Weighted sum
    const score = signals.fileOverlap * WEIGHT_FILE_OVERLAP +
    ...
    log.debug('Topic shift scored', {
        ...
        signals: Object.fromEntries(Object.entries(signals).map(([k, v]) => [k, v.toFixed(2)])),
        newTopic,
    });
