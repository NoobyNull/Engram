 ...
 */
export async function sdkSummarizeSession(sessionId) {
 ...
 */
export async function sdkSummarizeConversation(conversationId) {
 ...
 */
export async function sdkExtractKnowledge(sessionId) {
    const observations = getObservationsBySession(sessionId);
...
function buildDigest(observations) {
    ...
    for (const obs of observations.slice(0, 40)) {
        ...
        if (obs.tool_input_summary)
            parts.push(obs.tool_input_summary);
        if (obs.tool_output_summary)
            parts.push(obs.tool_output_summary.slice(0, 200));
