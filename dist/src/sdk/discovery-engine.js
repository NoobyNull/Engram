...
import { createEdge, findConnected } from '../db/knowledge-graph.js';
 ...
 */
export async function onKnowledgeCreated(knowledge) {
    ...
    try {
        ...
        for (const result of related) {
            if (result.id === knowledge.id)
                continue;
            if (result.score < 0.5)
                continue;
            // Don't duplicate edges we already created above
            if (knowledge.source_knowledge_ids.includes(result.id))
                continue;
...
async function tryDiscoverFromCombinations(knowledge) {
    ...
    if (!['fact', 'pattern', 'issue', 'discovery'].includes(knowledge.type))
        return;
...
function parseDiscoveries(text) {
    try {
        ...
        if (!Array.isArray(parsed.discoveries))
            return [];
        return parsed.discoveries.filter(d => typeof d.content === 'string' &&
            d.content.length > 0 &&
