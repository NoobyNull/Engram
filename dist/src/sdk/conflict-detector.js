 ...
 */
export async function detectConflict(observation) {
    const config = getConfig();
    if (config.conflictDetection?.enabled === false)
        return null;
    // Build a search query from the observation content
    const queryParts = [];
    if (observation.tool_input_summary)
        queryParts.push(observation.tool_input_summary);
    if (observation.tool_output_summary)
        queryParts.push(observation.tool_output_summary);
    const query = queryParts.join(' ').slice(0, 300);
    if (query.length < 15)
        return null; // Too short to meaningfully match
    try {
 ...
 */
export function buildConflictPrompt(conflict, observation) {
