...
export function storeEmbedding(sourceType, sourceId, text, embedding) {
    if (!isVectorsAvailable())
        return false;
    const db = getDb();
    ...
    try {
        ...
        const existing = db.prepare('SELECT id, text_hash FROM embeddings WHERE source_type = ? AND source_id = ?').get(sourceType, sourceId);
        if (existing) {
    ...
    `).run(id, sourceType, sourceId, textHash, now);
        ...
        db.prepare('INSERT INTO vec_embeddings (id, embedding) VALUES (?, ?)').run(id, Buffer.from(embeddingBuffer));
...
export function searchByVector(queryEmbedding, limit = 20) {
    if (!isVectorsAvailable())
        return [];
    const db = getDb();
    try {
...
export function deleteEmbedding(sourceType, sourceId) {
    if (!isVectorsAvailable())
        return false;
    const db = getDb();
    try {
